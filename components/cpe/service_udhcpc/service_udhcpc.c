...
void v_secure_system(char const *fmt, ...) {
    char str[256];
    va_list args;
...
#define OnboardLog(...) printf(__VA_ARGS__)
...
#endif
#define IP_UTIL_BIN_FULL_PATH "/sbin/ip"
...
static int get_and_fill_env_data (ipc_dhcpv4_data_t *dhcpv4_data, udhcpc_script_t* pinfo)
{
    ...
    if ( (strcmp(pinfo->input_option, "bound") == 0) || (strcmp(pinfo->input_option, "renew") == 0))
    {
        ...
        {
            strncpy(dhcpv4_data->ntpServer, env, sizeof(dhcpv4_data->ntpServer)-1);
            if(strlen(env)>sizeof(dhcpv4_data->ntpServer))
            {
                char *ptr_b=strrchr(dhcpv4_data->ntpServer,' ');
                if(ptr_b != NULL)
                {
                    *ptr_b = '\0';
                }
